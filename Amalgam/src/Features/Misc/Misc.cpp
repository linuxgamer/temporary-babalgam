...
void CMisc::RunPost(CTFPlayer* pLocal, CUserCmd* pCmd)
{
	...
	else
	{
		...
		if (!m_bEdgeBug)
		{
			...
			AutoHeatmakerFocus(pLocal, pCmd);
			BreakShootSound(pLocal, pCmd);
			MovementLock(pLocal, pCmd);
		}
	}

	// Movement recorder runs LAST so it captures the final movement command.
	F::Recorder.Run(pLocal, pCmd);
}
...
{
	if (!Vars::Misc::Movement::BotUtils::AutoHeatmakerFocus.Value || !pLocal || !pCmd)
		return;

	auto pWeapon = H::Entities.GetWeapon();
	if (!pWeapon || pWeapon->m_iItemDefinitionIndex() != Sniper_m_TheHitmansHeatmaker)
		return;
	if (pLocal->m_bRageDraining() || pLocal->InCond(TF_COND_SNIPERCHARGE_RAGE_BUFF))
		return;
	if (pLocal->m_flRageMeter() < 100.f)
		return;
