...
void CCritHack::Draw(CTFPlayer* pLocal)
{
	...
	if (pLocal)
	{
		// the render thread can hold a stale weapon cache after switches or deletions, only trust it while it still is the active weapon
		if (!pWeapon || !pLocal->IsAlive() || pLocal->IsAGhost() || pLocal->m_hActiveWeapon().Get() != pWeapon)
		{
			tCache = {};
			flCurrentProgress = 0.f;
			return;
